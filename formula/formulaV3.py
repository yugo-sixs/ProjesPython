...
from PyQt5.QtWidgets import (
    ...
    QVBoxLayout, QHBoxLayout, QFormLayout, QLineEdit, QComboBox, QFileDialog, QMessageBox
)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.dosis_input = QLineEdit()
        self.dosis_input.setFixedSize(100, 20)
        self.alto_input = QLineEdit()
        self.alto_input.setFixedSize(100, 20)
        self.ancho_input = QLineEdit()
        self.ancho_input.setFixedSize(100, 20)
        self.potencia_input = QLineEdit()
        self.potencia_input.setFixedSize(100, 20)
        self.porcentaje_input = QLineEdit()
        self.porcentaje_input.setFixedSize(100, 20)
    ...
    def calcular_tiempo(self):
        ...
        try:
            ...
            self.dosis_input.setStyleSheet("border: 2px solid red;")
            if not primer_error:
                primer_error = self.dosis_input
            error_detectado = True

        try:
            alto = float(self.alto_input.text())
        except ValueError:
            self.alto_input.setStyleSheet("border: 2px solid red;")
            if not primer_error:
                primer_error = self.alto_input
            error_detectado = True

        try:
            ancho = float(self.ancho_input.text())
        except ValueError:
            self.ancho_input.setStyleSheet("border: 2px solid red;")
            if not primer_error:
                primer_error = self.ancho_input
            error_detectado = True

        try:
            potencia_total = float(self.potencia_input.text())
        except ValueError:
            self.potencia_input.setStyleSheet("border: 2px solid red;")
            if not primer_error:
                primer_error = self.potencia_input
            error_detectado = True

        try:
            porcentaje = float(self.porcentaje_input.text()) / 100.0
        except ValueError:
            self.porcentaje_input.setStyleSheet("border: 2px solid red;")
            if not primer_error:
                primer_error = self.porcentaje_input
            error_detectado = True

        if error_detectado:
            if primer_error:
                primer_error.setFocus()
            QMessageBox.critical(self, "Error de entrada",
                                 "Por favor, ingrese solo números válidos en todos los campos.")
            self.result_label.setText("Error: Ingrese solo números válidos.")
            self.pdf_button.setEnabled(False)
            return
    ...
    def exportar_pdf(self):
        ...
        pdf.ln(10)
        pdf.cell(200, 10, txt=f"Dosis: {self.dosis_input.text()} J/cm²", ln=True)
        pdf.cell(200, 10, txt=f"Alto: {self.alto_input.text()} cm", ln=True)
        pdf.cell(200, 10, txt=f"Ancho: {self.ancho_input.text()} cm", ln=True)
        pdf.cell(200, 10, txt=f"Superficie tratada: {self.superficie:.2f} cm²", ln=True)
        pdf.cell(200, 10, txt=f"Potencia total: {self.potencia_input.text()} W", ln=True)
        pdf.cell(200, 10, txt=f"Porcentaje trabajo: {self.porcentaje_input.text()}%", ln=True)
